        ...
        <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle className="font-display flex items-center gap-2">
              <CalendarDays className="w-5 h-5 text-gold" /> Agendar Cita
            </DialogTitle>
            <DialogDescription>
          ...
          <div className="space-y-5">
            ...
            <div className="space-y-3">
              <h3 className="text-sm font-semibold flex items-center gap-2"><User className="w-4 h-4 text-gold" /> Datos del Paciente</h3>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs font-medium mb-1">Nombre *</label>
                  <input type="text" className="w-full bg-muted rounded-lg px-3 py-2.5 text-sm border border-border focus:border-gold focus:outline-none" value={form.patientName} onChange={(e) => update("patientName", e.target.value.replace(/[^a-zA-ZáéíóúÁÉÍÓÚñÑüÜ\s]/g, ""))} maxLength={50} />
                </div>
                <div>
                  <label className="block text-xs font-medium mb-1">Apellido *</label>
                  <input type="text" className="w-full bg-muted rounded-lg px-3 py-2.5 text-sm border border-border focus:border-gold focus:outline-none" value={form.patientLastName} onChange={(e) => update("patientLastName", e.target.value.replace(/[^a-zA-ZáéíóúÁÉÍÓÚñÑüÜ\s]/g, ""))} maxLength={50} />
                </div>
              </div>
              <div>
                <label className="block text-xs font-medium mb-1 flex items-center gap-1"><CreditCard className="w-3 h-3" /> Cédula *</label>
                <input type="text" inputMode="numeric" className="w-full bg-muted rounded-lg px-3 py-2.5 text-sm border border-border focus:border-gold focus:outline-none" value={form.patientCedula} onChange={(e) => update("patientCedula", e.target.value.replace(/[^0-9]/g, ""))} maxLength={20} placeholder="12345678" />
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs font-medium mb-1 flex items-center gap-1"><Mail className="w-3 h-3" /> Email *</label>
                  <input type="email" className="w-full bg-muted rounded-lg px-3 py-2.5 text-sm border border-border focus:border-gold focus:outline-none" value={form.patientEmail} onChange={(e) => update("patientEmail", e.target.value)} maxLength={100} placeholder="correo@ejemplo.com" />
                </div>
                <div>
                  ...
                  <div className="flex">
                    ...
                    <input type="tel" inputMode="numeric" className="w-full bg-muted rounded-r-lg rounded-l-none px-3 py-2.5 text-sm border border-border focus:border-gold focus:outline-none" value={form.patientPhone} onChange={(e) => { let val = e.target.value.replace(/[^0-9]/g, ""); if (val.startsWith("0")) val = val.slice(1); update("patientPhone", val); }} maxLength={10} placeholder="4121234567" />
                  </div>
                </div>
              </div>
            </div>
            ...
            {/* Appointment Details */}
            <div className="space-y-3">
              <h3 className="text-sm font-semibold flex items-center gap-2"><Stethoscope className="w-4 h-4 text-gold" /> Detalles de la Cita</h3>
              <div>
                <label className="block text-xs font-medium mb-1">Doctor *</label>
                <select className="w-full bg-muted rounded-lg px-3 py-2.5 text-sm border border-border focus:border-gold focus:outline-none" value={effectiveDoctorId} onChange={(e) => update("doctorId", e.target.value)}>
                  {doctors.map((d) => (
              ...
              <div>
                ...
                <select className="w-full bg-muted rounded-lg px-3 py-2.5 text-sm border border-border focus:border-gold focus:outline-none" value={effectiveTreatment} onChange={(e) => update("treatment", e.target.value)}>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input type="date" min={caracasToday} className="w-full bg-muted rounded-lg px-3 py-2.5 text-sm border border-border focus:border-gold focus:outline-none" value={form.date} onChange={(e) => { update("date", e.target.value); update("time", ""); }} />
                </div>
                <div>
                  <label className="block text-xs font-medium mb-1 flex items-center gap-1"><Clock className="w-3 h-3" /> Hora *</label>
                  <select className="w-full bg-muted rounded-lg px-3 py-2.5 text-sm border border-border focus:border-gold focus:outline-none" value={form.time} onChange={(e) => update("time", e.target.value)}>
              ...
              {form.date && (
                <button
                  ...
                  className="w-full flex items-center justify-center gap-2 text-xs text-muted-foreground hover:text-gold transition-colors py-2.5 border border-dashed border-border rounded-lg hover:border-gold/50"
                >
                  <HelpCircle className="w-3.5 h-3.5" />
                  <span>¿No encuentras horario? <strong className="text-gold">Solicitar uno personalizado</strong></span>
                </button>
              )}
              <div>
                <label className="block text-xs font-medium mb-1">Notas (opcional)</label>
                <textarea className="w-full bg-muted rounded-lg px-3 py-2.5 text-sm border border-border focus:border-gold focus:outline-none resize-none" rows={2} value={form.notes} onChange={(e) => update("notes", e.target.value)} maxLength={500} />
              </div>
            </div>
