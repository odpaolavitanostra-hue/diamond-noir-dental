...
import { DollarSign, Download, Plus, Trash2, BookOpen, ShoppingCart, Receipt } from "lucide-react";
...
export const AdminFinances = () => {
  ...
  const savePurchases = (items: AccountingEntry[]) => {
    setPurchases(items);
    localStorage.setItem("coso-purchases", JSON.stringify(items));
  };
  const saveSales = (items: AccountingEntry[]) => {
    setSales(items);
    localStorage.setItem("coso-sales", JSON.stringify(items));
  };

  const addEntry = (type: "compras" | "ventas") => {
    if (!newEntry.date || !newEntry.description || !newEntry.amountUSD) {
      toast.error("Completa fecha, descripción y monto");
      return;
    }
  ...
  const exportXLSX = () => {
    ...
    const purchaseData = purchases.map((p) => ({
      ...
      "Monto USD": p.amountUSD.toFixed(2),
      "Monto VES": (p.amountUSD * tasaBCV).toFixed(2),
    }));
    ...
    const salesData = sales.map((s) => ({
      ...
      "Monto USD": s.amountUSD.toFixed(2),
      "Monto VES": (s.amountUSD * tasaBCV).toFixed(2),
    }));
  ...
  const renderEntryForm = (type: "compras" | "ventas") => (
    <div className="bg-muted rounded-lg p-4 space-y-3 mb-4">
      ...
      <div className="grid grid-cols-2 gap-3">
        <input
          type="date"
          className="bg-card rounded-lg px-3 py-2 text-sm border border-border"
          value={newEntry.date}
      ...
      <input
        ...
        className="w-full bg-card rounded-lg px-3 py-2 text-sm border border-border"
      ...
      <div className="flex gap-3">
        <input
          ...
          className="flex-1 bg-card rounded-lg px-3 py-2 text-sm border border-border"
          value={newEntry.amountUSD}
          onChange={(e) => setNewEntry({ ...newEntry, amountUSD: e.target.value })}
        />
        <button
          ...
          .map((entry) => (
            <div key={entry.id} className="bg-card rounded-xl p-4 gold-border flex items-start justify-between gap-3">
              ...
              <div className="text-right flex items-center gap-2">
                ...
                <button onClick={() => deleteEntry(type, entry.id)} className="p-1.5 rounded-lg bg-clinic-red/10 text-clinic-red hover:bg-clinic-red/20">
    ...
    <div>
      ...
      <div className="flex gap-2 mb-6 flex-wrap">
        {[
          ...
          { key: "compras" as const, label: "Libro de Compras", icon: <ShoppingCart className="w-4 h-4" /> },
          { key: "ventas" as const, label: "Libro de Ventas", icon: <Receipt className="w-4 h-4" /> },
        ].map((tab) => (
          <button
            key={tab.key}
            onClick={() => setActiveTab(tab.key)}
            className={`flex items-center gap-1.5 px-4 py-2 rounded-lg text-sm font-medium transition-colors ${
              activeTab === tab.key ? "bg-gold text-gold-foreground" : "bg-card gold-border hover:bg-muted"
            }`}
          >
            {tab.icon} {tab.label}
          </button>
      ...
      {activeTab === "compras" && (
        <>
          {renderEntryForm("compras")}
          {renderEntries(purchases, "compras")}
        </>
      )}

      {activeTab === "ventas" && (
        <>
          {renderEntryForm("ventas")}
          {renderEntries(sales, "ventas")}
        </>
      )}
    </div>
  );
