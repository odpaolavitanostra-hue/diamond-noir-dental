...
export const AdminTenants = () => {
  const { tenants, addTenant, updateTenant, deleteTenant, addTenantBlockedSlot, removeTenantBlockedSlot } = useClinicData();
  ...
  const handleAddBlock = async (tenantId: string) => {
          ...
          <div key={t.id} className="bg-card rounded-xl p-5 gold-border space-y-3">
            ...
            {blockingTenant === t.id && (
              <div className="bg-muted rounded-lg p-4 space-y-3">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div><label className="block text-xs font-medium mb-1">Fecha</label><input type="date" className="w-full bg-card rounded-lg px-3 py-2 text-sm border border-border" value={blockForm.date} onChange={(e) => setBlockForm({ ...blockForm, date: e.target.value })} /></div>
                  <div className="flex items-end gap-2 pb-0.5"><label className="flex items-center gap-2 text-sm cursor-pointer"><input type="checkbox" checked={blockForm.allDay} onChange={(e) => setBlockForm({ ...blockForm, allDay: e.target.checked })} className="rounded" />Día completo</label></div>
                </div>
                {!blockForm.allDay && (
                  <div className="grid grid-cols-2 gap-3">
                    <div><label className="block text-xs font-medium mb-1">Desde</label><select className="w-full bg-card rounded-lg px-3 py-2 text-sm border border-border" value={blockForm.startTime} onChange={(e) => setBlockForm({ ...blockForm, startTime: e.target.value })}>{Array.from({ length: 9 }, (_, i) => 8 + i).map((h) => <option key={h} value={`${h.toString().padStart(2, "0")}:00`}>{h}:00</option>)}</select></div>
                    <div><label className="block text-xs font-medium mb-1">Hasta</label><select className="w-full bg-card rounded-lg px-3 py-2 text-sm border border-border" value={blockForm.endTime} onChange={(e) => setBlockForm({ ...blockForm, endTime: e.target.value })}>{Array.from({ length: 9 }, (_, i) => 9 + i).map((h) => <option key={h} value={`${h.toString().padStart(2, "0")}:00`}>{h}:00</option>)}</select></div>
                  </div>
                )}
