...
export const AdminDoctors = () => {
  ...
  const [form, setForm] = useState<{ name: string; email: string; specialty: string; payModel: 'fixed' | 'percent'; rate: number }>({
    name: "", email: "", specialty: "Odontología General", payModel: "percent", rate: 0.4,
  });
  ...
  const handleAdd = async () => {
    ...
    setAdding(false);
    setForm({ name: "", email: "", specialty: "Odontología General", payModel: "percent", rate: 0.4 });
  ...
  const startEdit = (d: Doctor) => {
    setEditing(d.id);
    setForm({ name: d.name, email: d.email, specialty: d.specialty, payModel: d.payModel, rate: d.rate });
  };
