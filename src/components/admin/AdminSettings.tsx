    ...
    <div className="space-y-6">
      ...
      <div className="bg-card rounded-xl p-5 gold-border space-y-3">
        <h3 className="font-semibold flex items-center gap-2">
          <Stethoscope className="w-4 h-4 text-primary" /> Precios de Servicios (USD)
        </h3>
  ...
  return (
    <div className="flex items-center gap-3 flex-wrap">
      ...
      <div className="flex items-center gap-2">
        ...
        <input type="number" step="0.01" min="0" className="bg-muted rounded-lg px-3 py-2 text-sm border border-border w-24 text-center" value={price} onChange={(e) => setPrice(e.target.value)} disabled={treatment.name === "Otros"} />
        <button onClick={handleSave} className="bg-primary text-primary-foreground px-3 py-2 rounded-lg text-xs font-semibold" disabled={treatment.name === "Otros"}>
